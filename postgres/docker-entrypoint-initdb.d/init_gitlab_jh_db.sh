...
if [ "$GITLAB_JH_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $GITLAB_JH_POSTGRES_USER WITH PASSWORD '$GITLAB_JH_POSTGRES_PASSWORD';
		CREATE DATABASE $GITLAB_JH_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $GITLAB_JH_POSTGRES_DB TO $GITLAB_JH_POSTGRES_USER;
		ALTER ROLE $GITLAB_JH_POSTGRES_USER CREATEROLE SUPERUSER;
	EOSQL
	echo
fi
