...
if [ "$HARBOR_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $HARBOR_POSTGRES_USER WITH PASSWORD '$HARBOR_POSTGRES_PASSWORD';
		CREATE DATABASE $HARBOR_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $HARBOR_POSTGRES_DB TO $HARBOR_POSTGRES_USER;
		ALTER ROLE $HARBOR_POSTGRES_USER CREATEROLE SUPERUSER;
	EOSQL
	echo
fi
